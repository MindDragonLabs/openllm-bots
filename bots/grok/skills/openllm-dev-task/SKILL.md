...
description: Execute a real coding or development task with Grok Bot as orchestrator using OpenLLM for models, gateway chat/completions, search, and optional context/memory MCP. Use when the user wants implementation work, not only account inspection.
---

# Development execution mode

**You** (Grok Bot) own the plan, the repo, tests, git, and deploy connectors.
...
Shared architecture: [shared/architecture.md](../../../../shared/architecture.md). If MCP is missing: **openllm-connect** / [docs/setup.md](../../docs/setup.md).
...
If MCP is not connected, run **openllm-connect** first.
...
Edit, lint, test, and commit using Grok Bot coding tools (editor, shell, git).
...
- Putting `OPENLLM_API_KEY` in source, CI logs, README samples, or shared skills
