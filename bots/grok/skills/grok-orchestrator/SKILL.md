...
- Cursor's `mcp.json` / the Cursor plugin's stdio server **does not automatically appear** on Grok Bot. Re-add the connector here.
- Grok Bot **cannot call localhost** on the user's laptop. Do not use `http://127.0.0.1:…` as the MCP URL.

Treat custom-connector setup as required until tools actually list.
...
3. **Import skills** from [`bots/grok/skills/`](https://github.com/MindDragonLabs/openllm-bots/tree/main/bots/grok/skills) or paste the prose into Grok Bot skills. Minimum set: `getting-started`, `openllm-connect`, `openllm-dashboard`, `openllm-dev-task`, `grok-orchestrator`.
