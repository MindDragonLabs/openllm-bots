...
This folder is **reserved** for the **Muse** orchestrator.

Muse (when published) is the orchestrator. [OpenLLM](https://openllm.sh) is the **same model fabric** as every other bot in this monorepo: `openllm mcp`, variables `OPENLLM_API_KEY` and `OPENLLM_CLOUD_ORIGIN`.
